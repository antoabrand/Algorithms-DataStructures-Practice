...
public class LinkedListDemoApp {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "This small app will take a method name and perform some action on a list given a number.");
    ...
    while (keepGoing > 0)
      try {
        ...
        System.out.println(
            "Your input is wrong. Fix it! (Hint...it should be a number like was asked for :)~ )");
      }
   ...
   * 
   **/
  private static void changeNPrint(String methodName, LinkedList<Integer> myLinkedList,
      Integer num) {
 ...
 * 
 * ...Pros: dynamic, allocates needed memory in run-time, very efficient if we want to manipulate
 * the first Elements, stores items with different size ...Cons: wastes memory because of
 * references, nodes must be read from the beginning as they have sequential access, items reached
 * in O(1) - linear complexity, reverse traversing is difficult - Solution: doubly linked lists ->
 * easier to read, but memory is wasted in allocating space for a back pointer
 * 
 * Insertion/Removal of items -
 * 
 * linkedList.add() - add item to at end of list - not simple, need to traverse whole linked list to
 * find last node find node pointing to null, then update references when we get there O(N) time
 * complexity linkedList.push() - O(1) - because it will always only have to iterate once at the
 * beginning of the list linkedList.removeFirst()- removing an item from beginning of list - O(1)
 * linkedList.remove() - an item from anywhere else - O(N)
 *
 *
 * 
 * Problems with LinkedLists - we can traverse forward but not backward due to references pointing
 * to next node Solution - doubly linkedLists -> Node class has 2 references - one to next node/ one
 * to previous con - heavy on memory - storing all the extra references - however, no need to track
 * previous node during traversal
 **/
